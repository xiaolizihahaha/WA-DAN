...
class WSDAN(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(attention_maps.shape)  # torch.Size([12, 32, 14, 14])
        ...
        if self.training:
            ...
            attention_map = torch.stack(attention_map)  # (B, 2, H, W) - one for cropping, the other for dropping  # torch.Size([12, 2, 14, 14])

        else:
            # Object Localization Am = mean(Ak)
            attention_map = torch.mean(attention_maps, dim=1, keepdim=True)  # (B, 1, H, W)  # torch.Size([12, 1, 14, 14])
